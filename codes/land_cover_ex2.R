...
beta_mnl <- list(
  grass = c(0.2, -0.5, 0.3, 1),
  crop = c(0.4, -0.2, 0.1, 1),
  urban = c(0, -0.5, 0.2, 2.5)
)
...
# Linear predictor for urban
pred_urban <- beta_mnl$urban[1]*X1_mnl + beta_mnl$urban[2]*X2_mnl + beta_mnl$urban[3]*X3_mnl + beta_mnl$urban[4]*X4_mnl


exp_forest <- 1 ##### why is this 1?
exp_grass <- exp(pred_grass)
exp_crop <- exp(pred_crop)
exp_urban <- exp(pred_urban)

sum_exp <- exp_forest + exp_grass + exp_crop + exp_urban
probs <- cbind(
  forest = exp_forest / sum_exp,
  grass = exp_grass / sum_exp,
  crop = exp_crop / sum_exp,
  urban =  exp_urban / sum_exp
)
...
# Sample class labels
Y_mnl <- apply(probs, 1, function(p) sample(1:4, 1, prob = p))
Y_mnl <- factor(Y_mnl, labels = c("forest", "grass", "crop", "urban"))
