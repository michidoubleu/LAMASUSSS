...
beta_mnl <- list(
  grass = c(0.2, -0.5, 0.3),   # intercept, X1, X2
  crop = c(0.4, -0.2, 0.1)     # intercept, X1, X2
)

# Calculate linear predictors and softmax probabilities

# Linear predictor for grass
pred_grass <- beta_mnl$grass[1]*X1_mnl + beta_mnl$grass[2]*X2_mnl + beta_mnl$grass[3]*X3_mnl

# How would the linar predictor for  crop look like?
pred_crop <- beta_mnl$crop[1]*X1_mnl + beta_mnl$crop[2]*X2_mnl + beta_mnl$crop[3]*X3_mnl
...
# Fit MNL model
data_mnl <- data.frame(Y = Y_mnl, X1 = X1_mnl, X2 = X2_mnl, X3 = X3_mnl)
model_mnl <- multinom(Y ~ X1 + X2 + X3 -1 , data = data_mnl, trace = FALSE)
